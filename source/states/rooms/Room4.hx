...
class Room4 extends BaseRoom
{
    ...
    override function create() 
    {
        ...
        var table2:FlxSprite = new FlxSprite().loadGraphic(Paths.image('map/table2'));
        table2.offset.y = 34;
        ...
        var table:FlxSprite = new FlxSprite().loadGraphic(Paths.image('map/table1'));
        ...
        var table3:FlxSprite = new FlxSprite().loadGraphic(Paths.image('map/table1'));
        ...
        var exit1:Exit = new Exit('Room2', wallTopLeft.width, 0, Std.int(FlxG.width - (wallTopLeft.width + wallTopRight.width)), 1, new FlxPoint(-1, FlxG.height - ourple.height - 2)); //Top Exit
        add(exit1);
        exits.push(exit1);

        var exit2:Exit = new Exit('Room5', 0, wallTopLeft2.height, 1, Std.int(FlxG.height - (wallTopLeft2.height + wallTopLeft2.height)), new FlxPoint(FlxG.width - ourple.width - 2, -1)); //Left Exit
        add(exit2);
        exits.push(exit2);

        var exit3:Exit = new Exit('Room7', FlxG.width - 2, wallTopRight2.height, 1, Std.int(FlxG.height - (wallTopRight2.height + wallTopRight2.height)), new FlxPoint(2, -1)); //Right Exit
        add(exit3);
        exits.push(exit3);
    }
}