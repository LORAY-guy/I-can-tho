...
class RoomCorridorLeft extends BaseRoom
{
    ...
    override function create() 
    {
        ...
        var exit2:Exit = new Exit('RoomCornerLeft', wallBottomLeft.width, FlxG.height - 1, Std.int(FlxG.width - (wallBottomLeft.width * 2)), 1, new FlxPoint(-1, 2)); //Bottom Exit
        add(exit2);
        exits.push(exit2);

        var exit3:Exit = new Exit('RoomSupplyCloset', 0, wallBottomLeft.height, 1, Std.int(FlxG.height - (wallBottomLeft.height + wallBottomLeft.height)), new FlxPoint(FlxG.width - ourple.width - 2, -1)); //Left Exit
        add(exit3);
        exits.push(exit3);
    }
}